...
double w1=3,w2=-5;//angular velocities
Vector2D s1(0,0.01),s2(0.5,0);//displacement wrt the fiducial point 2D
Vector2D v1(1,0),v2(-2,0);//velocities 2D
Vector2D scom,vcom;
//Vector2D s1(0,0),s2(1,0);//displacement wrt the fiducial point 1D
//Vector2D v1(1,0),v2(-2,0);//velocities 1D
double k=1500;//spring constant
...
void step(){
    ...
    if(dis<2*r){//check for contact
        ...
        double Ff=u*k*x;//magnitude of frictional force intersection of the two circles
    ...
    //calculates quantities related to the center of mass
